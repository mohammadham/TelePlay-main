...
class Track(Base):
    ...
    media_type: Mapped[str] = mapped_column(String(20), default="audio", nullable=False)  # audio, music_video, reel
    created_at: Mapped[datetime] = mapped_column(DateTime, default=datetime.utcnow)
    artist: Mapped["Artist"] = relationship(back_populates="tracks")
    album: Mapped[Optional["Album"]] = relationship(back_populates="tracks")
    __table_args__ = (Index("idx_track_artist", artist_id), Index("idx_track_album", album_id), Index("idx_track_genre", genre), Index("idx_track_popular", play_count), Index("idx_track_media_type", media_type),)
...
class BotConfig(Base):
    ...
    bot_user_id: Mapped[Optional[int]] = mapped_column(BigInteger, index=True)
    username: Mapped[Optional[str]] = mapped_column(String(100))
    purpose: Mapped[str] = mapped_column(String(20), default="MAIN")  # MAIN, HELPER, ADS, STORAGE
    is_active: Mapped[bool] = mapped_column(Boolean, default=True)
    rate_limit_remaining: Mapped[int] = mapped_column(Integer, default=30)
    ...
    __table_args__ = (
        Index("idx_bot_purpose", purpose),
        Index("idx_bot_created_by", created_by),
    )


class UserAccount(Base):
    """Telegram MTProto user account (for file storage/streaming)."""
    ...
    __table_args__ = (
        Index("idx_user_account_purpose", purpose),
        Index("idx_user_account_created_by", created_by),
    )


class AdminUser(Base):
    """Admin user hierarchy with role-based permissions."""
    ...
    first_name: Mapped[Optional[str]] = mapped_column(String(255))
    last_name: Mapped[Optional[str]] = mapped_column(String(255))
    role: Mapped[str] = mapped_column(String(20), default="ADMIN")  # SUPER_ADMIN, ADMIN, MODERATOR
    is_active: Mapped[bool] = mapped_column(Boolean, default=True)
