"""
Video domain API — Netflix-like browse, movies, series, episodes, progress
"""
...
def _movie_to_resp(m: Movie) -> dict:
    return {
        ...
        "genre": m.genre, "year": m.year, "file_id": m.file_id,
        "duration": m.duration, "featured": m.featured,
        "created_at": m.created_at,
        "stream_url": f"/api/stream/{m.file_id}",
...
async def list_movies(
    q: Optional[str] = None, genre: Optional[str] = None,
...
async def create_movie(payload: dict, db: AsyncSession = Depends(get_db), current_user: User = Depends(get_current_user)):
    ...
    m = Movie(
        ...
        genre=payload.get("genre"), year=payload.get("year"),
        file_id=payload["file_id"], duration=payload.get("duration"),
        featured=payload.get("featured", False), thumbnail_url=payload.get("thumbnail_url"),
    )
...
async def browse(db: AsyncSession = Depends(get_db), current_user: User = Depends(get_current_user)):
    hero = (await db.execute(
        select(Movie).where(Movie.featured == True).order_by(Movie.created_at.desc()).limit(1)
    )).scalar_one_or_none()

    # Continue watching
    cw_ids = (await db.execute(
        select(VideoProgress.file_id)
        .where(VideoProgress.user_id == current_user.id, VideoProgress.completed == False)
        .order_by(VideoProgress.updated_at.desc()).limit(10)
    )).scalars().all()
    cw = []
    if cw_ids:
        cw_rows = (await db.execute(select(Movie).where(Movie.file_id.in_(cw_ids)))).scalars().all()
        cw = [MovieResponse(**_movie_to_resp(m)).model_dump() for m in cw_rows]

    # By genre
    genres = (await db.execute(
        select(Movie.genre).where(Movie.genre.isnot(None)).distinct().limit(6)
    )).scalars().all()
    by_genre = {}
    for g in genres:
        if not g:
    ...
    return {
        "hero": MovieResponse(**_movie_to_resp(hero)).model_dump() if hero else None,
...
async def search_video(
    ...
    series = (await db.execute(
        select(Series).where(Series.title.ilike(f"%{q}%")).limit(20)
    )).scalars().all()
    return {
        "movies": [MovieResponse(**_movie_to_resp(m)).model_dump() for m in movies],
        "series": [SeriesResponse.model_validate(s, from_attributes=True).model_dump() for s in series],
    }


@router.post("/progress")
async def update_progress(
    payload: dict, db: AsyncSession = Depends(get_db), current_user: User = Depends(get_current_user)):
    file_id = payload["file_id"]
    row = (await db.execute(
        select(VideoProgress).where(VideoProgress.user_id == current_user.id, VideoProgress.file_id == file_id)
    )).scalar_one_or_none()
    if not row:
        row = VideoProgress(
...
async def get_progress(
    file_id: int, db: AsyncSession = Depends(get_db), current_user: User = Depends(get_current_user)):
    row = (await db.execute(
        select(VideoProgress).where(VideoProgress.user_id == current_user.id, VideoProgress.file_id == file_id)
    )).scalar_one_or_none()
    if not row:
        return {"position": 0, "duration": None, "completed": False}
