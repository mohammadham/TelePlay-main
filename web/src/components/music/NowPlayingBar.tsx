...
import { Play, Pause, SkipBack, SkipForward, Volume2, Repeat, Shuffle, Heart, Maximize2, Minimize2 } from 'lucide-react'
...
export default function NowPlayingBar() {
  ...
  return (
    <div className={`fixed left-0 right-0 bg-[#181818] border-t border-white/10 z-40 transition-all duration-300 ${isVideo ? (isMini ? 'bottom-0 h-14' : 'bottom-0 h-[320px]') : 'bottom-0 h-[84px]'}`}>
      {isVideo && !isMini && (
        <div className="relative w-full aspect-video bg-black">
          {isVideo ? (
            <video ref={mediaRef as any} src={src} className="w-full h-full object-contain"
          ...
          <div className="absolute top-2 right-2">
            <button onClick={(e)=>{e.stopPropagation(); setIsMini(true)}} className="bg-black/50 hover:bg-black/70 text-white p-1.5 rounded-full transition">
      ...
      {/* Controls bar */}
      <div className={`flex items-center px-4 gap-4 ${isVideo && !isMini ? 'mt-2' : ''}`}>
        <div className={`flex items-center gap-3 ${isVideo && !isMini ? 'w-[30%]' : 'w-[30%]'}`}>
        ...
        <div className="w-[30%] flex items-center justify-end gap-2">
          ...
          <input type="range" min={0} max={1} step={0.05} value={volume} onChange={e=>{ setVolume(parseFloat(e.target.value)); if(mediaRef.current) (mediaRef.current as any).volume = parseFloat(e.target.value)}} className="w-24 accent-white" />
        </div>
      </div>
    </div>
  )
